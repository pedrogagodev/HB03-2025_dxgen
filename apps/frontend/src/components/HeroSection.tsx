  ...
  return (
    <section className="flex w-full flex-col items-center text-center md:max-w-3xl">
      <div className="flex flex-col items-center gap-3">
        <h1 className="bg-linear-to-b from-slate-50 to-slate-300 bg-clip-text text-4xl font-semibold tracking-tight text-transparent sm:text-5xl md:text-6xl">
          {title}
        </h1>
        {titleSuffix && (
      ...
      <form className="mt-8 flex w-full max-w-md flex-col items-center gap-3 sm:flex-row">
        <div className="flex w-full items-center rounded-full border border-slate-700/80 bg-slate-900/70 px-4 py-2 text-left text-sm text-slate-200 shadow-[0_18px_60px_rgba(15,23,42,0.9)] sm:py-2.5">
          <input
            type="email"
          ...
          <button
            type="submit"
            className="ml-3 hidden whitespace-nowrap rounded-full bg-emerald-400 px-4 py-1.5 text-xs font-semibold text-slate-900 hover:bg-emerald-300 sm:inline-flex"
          >
            {ctaLabel}
          </button>
        </div>
        <button
          type="submit"
          className="inline-flex w-full items-center justify-center rounded-full bg-emerald-400 px-4 py-2 text-xs font-semibold text-slate-900 hover:bg-emerald-300 sm:hidden"
        >
          {ctaLabel}
        </button>
      </form>
